...
from matting_inference_core import load_full_finetuned_weights, load_sft_transformer_checkpoint
...
class FluxTrainingModule(DiffusionTrainingModule):
    def __init__(
        ...
        self.pipe = FluxImagePipeline.from_pretrained(torch_dtype=torch.bfloat16, device="cuda", model_configs=model_configs,model_base_path=model_paths)
        if init_dit_checkpoint:
            if str(init_dit_checkpoint).endswith("diffusion_pytorch_model.safetensors"):
                load_sft_transformer_checkpoint(self.pipe, init_dit_checkpoint)
            else:
                load_full_finetuned_weights(self.pipe, init_dit_checkpoint)
...
if __name__ == "__main__":
    ...
    if args.task in {"depth", "normal", "matting"}:
        print(f"!!! Doing {args.task} task !!!")
    elif "depth" in metadata_paths[0]:
        args.task = "depth"
        ...
        raise ValueError("Cannot infer task from metadata path; please pass --task depth, --task normal, or --task matting.")
    if args.using_pdf:
    ...
    if len(datasets_ls) > 1:  
        ...
        #     first_path = item[dataset.data_file_keys[0]]
        #     if "png" in os.path.basename(first_path).lower():
        #         png_indices.append(i)
        #     else:
        #         no_png_indices.append(i)
        
        # print(f"no-png total: {len(no_png_indices)}, png total: {len(png_indices)}")
        
        # # 按比例限制数量
        # no_png_target = 84000   # 70%
        # png_target    = 36000   # 30%
        
        # # 随机采样（不足则全取）
        # random.shuffle(no_png_indices)
        # random.shuffle(png_indices)
        # no_png_indices = no_png_indices[:no_png_target]
        # png_indices    = png_indices[:png_target]
        
        # print(f"Sampled: no-png={len(no_png_indices)}, png={len(png_indices)}")
        
        # subset_no_png = Subset(dataset, no_png_indices)
        # subset_png    = Subset(dataset, png_indices)
        
        # dataset = ConcatDataset([subset_no_png, subset_png])
        # mixed_sampler = MixedBatchSampler(
        #     [subset_no_png, subset_png],
        #     shuffle=True,
        #     batch_size=args.batch_size,
        #     drop_last=True,
        #     prob=[0.7, 0.3],
        # )
        # print(f"Total dataset size: {len(dataset)}, total batches: {len(mixed_sampler)}")
    print(args.eval_file_list)
    if args.eval_file_list:
        with open(args.eval_file_list, "r") as f:
            eval_file_list = [line.strip().split()[0] for line in f if line.strip()]
            if not args.eval_base_path:
                raise ValueError("--eval_base_path is required when --eval_file_list contains relative evaluation paths.")
            base_dir = args.eval_base_path
    ...
    model = FluxTrainingModule(
        ...
        matting_prompt=args.matting_prompt if args.task == "matting" else None,
        init_dit_checkpoint=args.init_dit_checkpoint,
    )
