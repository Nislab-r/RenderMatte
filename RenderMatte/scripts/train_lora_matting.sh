...
: "${OUTPUT_DIR:?Set OUTPUT_DIR to the LoRA checkpoint output directory}"

PYTHON_BIN="${PYTHON_BIN:-python}"
ACCELERATE_CONFIG="${ACCELERATE_CONFIG:-configs/accelerate_config.yaml}"
INIT_DIT_CHECKPOINT="${INIT_DIT_CHECKPOINT:-}"
...
LORA_TARGET_MODULES="${LORA_TARGET_MODULES:-a_to_qkv,b_to_qkv,ff_a.0,ff_a.2,ff_b.0,ff_b.2,a_to_out,b_to_out,proj_out,norm.linear,norm1_a.linear,norm1_b.linear,to_qkv_mlp}"
EVAL_LIST="${EVAL_LIST:-}"
EVAL_ROOT="${EVAL_ROOT:-}"
...
if [[ -n "$INIT_DIT_CHECKPOINT" ]]; then
  cmd+=(--init_dit_checkpoint "$INIT_DIT_CHECKPOINT")
fi
