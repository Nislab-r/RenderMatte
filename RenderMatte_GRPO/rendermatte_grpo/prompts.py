...
# IE = inflect.engine()
IE=None
ASSETS_PATH = resources.files("rendermatte_grpo.assets")
...
def _load_lines(path):
    ...
    `rendermatte_grpo/assets` directory for a file named `path`.
    """
    if not os.path.exists(path):
        newpath = ASSETS_PATH.joinpath(path)
    if not os.path.exists(newpath):
        raise FileNotFoundError(f"Could not find {path} or rendermatte_grpo.assets/{path}")
    path = newpath
